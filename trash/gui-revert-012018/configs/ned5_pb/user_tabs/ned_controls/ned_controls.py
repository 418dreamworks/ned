...
class UserTab(QWidget):
    def __init__(self, parent=None):
        ...
        # the ini owns the fork count; the class attribute is only a fallback
        self.RACK_TABLE_FORKS = self._rack_forks()
        LOG.info('RACK: %d fork(s) from [EMCIO]POCKETS', self.RACK_TABLE_FORKS)
        ...
        QTimer.singleShot(1700, self._drop_conversational)
    ...
    def _build_subtabs(self):
        ...
        try:
            ...
            rbox, rbl = _mkbox('RACK CALIBRATION  --  P1..P%d fork centres'
                               % self.RACK_TABLE_FORKS)
            ...
            # ---- ONE FORK, wherever it is ------------------------
            # Operator 2026-08-16, adding P15: "its situated further
            # away, so how do i probe it? its not in the linear
            # patterm" ... "can we add a special button that just
            # probes and updates 1 tool". START RACK CAL walks the
            # rack on a fixed 92.0 mm pitch, so a holder off that
            # pitch is unreachable by it -- the predicted centre
            # lands in air and, every descent being a probe, the
            # cycle aborts without ever measuring it.
            # This calls the SAME rc_locate with the predicted centre
            # taken from where the operator parked. One fork is
            # written; no other 4100 block is touched.
            obox, obl = _mkbox('ONE FORK  --  probe where the spindle is parked')
            orow = QHBoxLayout()
            orow.setSpacing(8)
            olab = QLabel('FORK')
            olab.setStyleSheet('color: rgb(200,200,200); font: 14pt "Probe Basic Bebas Mono";')
            orow.addWidget(olab)
            ospin = QSpinBox()
            ospin.setObjectName('cal_rack_one_fork')
            ospin.setRange(1, max(1, int(self.RACK_TABLE_FORKS)))
            ospin.setValue(max(1, int(self.RACK_TABLE_FORKS)))
            ospin.setMinimumHeight(46)
            ospin.setStyleSheet(
                'QSpinBox { background: #1B1F20; border: 1px solid #3A4244;'
                ' color: #DCE3E0; border-radius: 2px; padding: 2px 6px;'
                ' font: 16pt "Probe Basic Bebas Mono"; min-width: 90px; }'
                'QSpinBox:disabled { color: #5D6567; border: 1px solid #2A3133; }')
            self._rack_one_spin = ospin
            orow.addWidget(ospin)
            orow.addStretch(1)
            obl.addLayout(orow)
            obtn = QPushButton('PROBE THIS FORK ONLY')
            obtn.setObjectName('cal_rack_one_btn')
            obtn.setMinimumHeight(62)
            obtn.setStyleSheet(self.CAL_QSS.get('go', ''))
            obtn.clicked.connect(lambda _=False: self._cal_run('rackone'))
            self._cal_btn['rackone'] = obtn
            obl.addWidget(obtn)
            onote = QLabel('Empty spindle, holder energized. Park '
                           'CENTRED 10 mm above THAT holder top and '
                           'zero work XYZ there. Writes that fork '
                           'only.')
            onote.setWordWrap(True)
            onote.setStyleSheet('color: rgb(160,160,160); font: 12pt "Probe Basic Bebas Mono";')
            obl.addWidget(onote)
            rl.addWidget(obox)
            LOG.info('RACK CAL ONE: button built, forks 1..%d',
                     self.RACK_TABLE_FORKS)
            ...
            for k, b in getattr(self, '_cal_btn', {}).items():
                ...
                if busy and k == running:
                    ...
                    if k == 'clear':
                        ...
                        cls = 'measure'
                    b.setStyleSheet(self.CAL_QSS[cls])
            # the fork selector is part of that control: a number the
            # operator can still change mid-cycle is a lie about what
            # is being probed
            sp = getattr(self, '_rack_one_spin', None)
            if sp is not None:
    ...
    CAL_SUBS = {
        ...
        # ONE FORK: same rc_locate, centre = the parked pose, and it
        # takes the fork number as its single argument.
        'rackone': ('rack_cal_one', 'RACK CAL ONE', False),
    }
        ...
        try:
            ...
            if need_centre:
                ...
                if which == 'probeheight':
                    ...
                    sp = getattr(self, '_rack_one_spin', None)
                    if sp is None:
                        c.error_msg('RACK CAL ONE refused: fork selector '
                                    'was never built -- nothing issued')
                        LOG.error('RACK CAL ONE: _rack_one_spin missing')
                        self._hand_back_manual(c, label)
                        self._cal_issue_failed(which)
                        return
                    plain = ' [%d]' % int(sp.value())
    ...
    CAL_MSG = {
        ...
        'rack': 'RACK CAL running: walks the rack from the P1 pose on a '
                'fixed 92.0 mm Y pitch, probing every fork.',
        'rackone': 'RACK CAL ONE running: probes the ONE fork selected '
                'above, centred on the parked pose. No other fork is '
                'touched.',
    ...
    def _b_side_gate_tick(self):
        ...
        frozen side to catch up at full velocity into a loaded worm, by
        however far the two have been driven apart -- and since 2026-08-16
        there is no bound on that at all (the max-split pin was deleted:
        "user will decide how much is too much"). Unwinding is a MOTION and
        stays the operator's, on the same toggle that built it.
        """
    ...
    def _split_toolchange_panel(self):
        ...
        try:
            ...
            # AND SHORTER THAN ITS ROW. Operator 2026-08-16: "the box the 15
            # is in needs to be smaller proportional to row height". The
            # widget rect was already the same 45 px as the GET TOOL button
            # beside it -- measured off the screenshot, entry and call both
            # QRect(...,45) -- but the button paints a dark bevel inset in its
            # rect while a QLineEdit paints a solid light fill edge to edge.
            # Equal rects, so a near-square light slab against a wide dark
            # button, which is what read as oversized. Every other light field
            # in this window sits INSET in its row: the ATC TOOL LENGTH and
            # DIAM fields paint 42 px and the bottom-panel LENGTH field 33 px.
            # Two thirds of the row puts it in that company and keeps the
            # relationship proportional, as asked, rather than a pixel I
            # picked. QHBoxLayout centres a fixed-height item vertically.
            entry.setFixedHeight(int(ROW_H * 2 / 3))
            if tlab is not None:
    ...
    def _drop_conversational(self):
        """Remove the CONVERSATIONAL tab from the main tab bar.

        Operator 2026-08-17: "remove the entire conversational tab compeltely.
        that should be skills. and I much prefer it to be skills" ... "we will
        rebuilt it from skills instead of this programmatic shit".

        REMOVING THE PAGE IS NOT ENOUGH. Hiding conversational_tab leaves its
        BUTTON on the bar -- QTabWidget draws the tab from its own index, not
        from the page's visibility -- so the operator can still land on an
        empty page. The tab has to come off by index.

        Found by walking up from the page to whichever QTabWidget owns it,
        rather than naming the bar: the stock .ui has several (tabWidget,
        tabWidget_2) and which one holds this page is a PB-version detail we
        should not bake in.
        """
        from PySide6.QtWidgets import QTabWidget
        win = self.window()
        page = win.findChild(QWidget, 'conversational_tab') if win else None
        if page is None:
            # absent is the INTENDED end state; only say so quietly
            LOG.info('CONVERSATIONAL: tab already absent -- nothing to remove')
            return
        bar = page.parentWidget()
        while bar is not None and not isinstance(bar, QTabWidget):
            bar = bar.parentWidget()
        if bar is None:
            LOG.error('CONVERSATIONAL: page found but no QTabWidget owns it '
                      '-- tab NOT removed, it is still reachable')
            return
        i = bar.indexOf(page)
        if i < 0:
            LOG.error('CONVERSATIONAL: page is not a tab of %s -- NOT removed',
                      bar.objectName() or '?')
            return
        bar.removeTab(i)
        page.hide()
        page.deleteLater()
        LOG.info('CONVERSATIONAL: tab removed from %s at index %d, %d tab(s) '
                 'left -- rebuild as skills', bar.objectName() or '?', i,
                 bar.count())
    ...
    # FORK COUNT COMES FROM THE INI, NOT A LITERAL. [EMCIO]POCKETS is 15 and
    # #3973 (what toolchange.ngc actually checks against) is 15, but this
    # constant and the tool table's P validation both said 14 -- so a
    # fifteenth holder could be neither given a home nor drawn, while the
    # g-code was perfectly willing to use it. Read it once; fall back to 14
    # only if the ini cannot be read at all.
    @staticmethod
    def _rack_forks(default=14):
        try:
            import linuxcnc
            ini = linuxcnc.ini(os.environ['INI_FILE_NAME'])
            # [ATC], not [EMCIO] -- verified against the live ini, which
            # returned None for [EMCIO] and would have fallen back to 14
            # silently, which is the bug this is meant to remove.
            n = ini.find('ATC', 'POCKETS')
            return int(float(n)) if n else default
        except Exception:
            return default

    RACK_TABLE_FORKS = 14          # replaced at construction by _rack_forks()
    ...
    def _sync_tool_safety(self):
        ...
        try:
            ...
            for tno, name, val in rows:
                # safety block is sized 1..<fork count>; home_p mirrors up
                # to T30 (T15 the sawblade MUST keep its assigned home).
                # 14 was a literal here, so T15's SAFETY X never reached
                # #[4200+2*15] and m21 fell back to the global clearance.
                # 4200+2T for T=15 is #4230/#4231 and the next block starts
                # at 4400, so the extra pair collides with nothing.
                cap = 30 if name == 'home_p' else self.RACK_TABLE_FORKS
...
"""
    ...
    def ac_to_zero(self, ax):
        ...
        #    Completing a home while motion is in FREE makes motion
        #    switch itself to teleop:
        #        control.c:261-263  if (motion_state == EMCMOT_MOTION_FREE
        #                               && do_homing()) switch_to_teleop_mode();
        #        motion.c:163-181   switch_to_teleop_mode -> teleoperating = 1
        #    That is deliberate upstream -- it is how the MPG goes live the
        #    instant homing finishes. But it means the teleop_enable(0)
        #    above, issued BEFORE c.home(jn), cannot survive c.home(jn),
        #    and the joint jog below is then refused outright:
        #        command.c:425      "Mode is TELEOP, cannot jog joint"
        #    The refusal is an operator-error message, not a return code, so
        #    ac_to_zero returned True and the Home All chain saw only that A
        #    never arrived (2026-08-16: A stuck at -89.9910 every attempt,
        #    Home All / Home A / Home C / Home A&C / the -xyzab launch
        #    sequence alike -- they all come through this one line).
        #    C escaped it only by sitting inside the 0.001 deg early return.
        c.teleop_enable(0)
        c.wait_complete(2.0)
        # VERIFY, do not assume: teleop_enable is honoured by motion on a
        # later servo cycle (control.c:962 needs INPOS), so wait_complete()
        # returning proves the command was accepted, not that the mode
        # changed. Measured 2026-08-16: FREE -> unhome -> home -> TELEOP.
        def _joint_mode():
            s.poll()
            return s.motion_mode != linuxcnc.TRAJ_MODE_TELEOP
        if not self._wait_for(_joint_mode, 2.0, '%s joint mode' % label,
                              poll=0.05):
            c.error_msg('%s refused: motion stayed in TELEOP after the '
                        'adopt -- a joint jog would be discarded, so '
                        'nothing was sent. %s is still at %+.4f'
                        % (label, ax.upper(), here))
            LOG.error('%s: motion_mode still TELEOP after the adopt -- '
                      'NOT jogging joint %d, still at %+.4f',
                      label, jn, here)
            return False
