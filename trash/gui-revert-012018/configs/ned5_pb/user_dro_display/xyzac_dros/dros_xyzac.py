...
class UserDRO(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        QTimer.singleShot(1400, self._target_column)
    ...
    def _target_column(self):
        """Third column: TARGET -- where the axis is going, not how far is left.

        Operator 2026-08-16: "i want DTG back, but instead of DTG, I want is as
        'Target' where we jog to instead of how far to go. make it 3 cols in
        both DROs, use the current workplace coordinate."

        DTG and Target are the same measurement read from opposite ends:
        LinuxCNC gives distance-to-go, and the operator wants the endpoint. So
        the number here is the COMMANDED position -- stat.position, which is
        where motion is heading -- expressed in the active work frame, the same
        frame the WORK column uses.

        WHY THE WIDGETS ARE REPLACED. The .ui cells are qtpyvcp DROLabels bound
        to a status channel; that binding repaints them on its own schedule and
        would fight anything written here -- this file already documents that
        for the unhomed case ("the stock binding resumes and overwrites"). A
        plain QLabel in the same grid slot has no binding to fight.
        """
        ...
        for a in ('x', 'y', 'z', 'a', 'c'):
            ...
            if w is None:
                missing.append(n)
                continue
            lay = w.parentWidget().layout() if w.parentWidget() else None
            lab = QLabel(w.parentWidget())
            lab.setObjectName('ned_target_' + a)
            lab.setMinimumSize(w.minimumSize())
            lab.setMaximumSize(w.maximumSize())
            lab.setAlignment(w.alignment())
            lab.setFont(w.font())
            lab.setStyleSheet(w.styleSheet())
            lab.setText('--')
            if lay is not None and hasattr(lay, 'replaceWidget'):
                lay.replaceWidget(w, lab)
            else:
                lab.setGeometry(w.geometry())
            w.hide()
            w.deleteLater()
            self._target_lbl[a] = lab
            made += 1
        h = self.findChild(QWidget, 'dtg_column_header')
        if h is None:
            win = self.window()
            h = win.findChild(QWidget, 'dtg_column_header') if win else None
        if h is not None:
            h.setText('TARGET')
            h.show()
        else:
            missing.append('dtg_column_header')
        LOG.info('TARGET column: %d cell(s) built, header %s%s', made,
                 'set' if h is not None else 'MISSING',
    ...
    def _unhomed_dro(self):
        ...
        try:
            ...
            return
        # TARGET COLUMN. Commanded position in the ACTIVE WORK FRAME -- the
        # same frame the WORK column shows, so the two read against each other
        # directly: WORK is where the axis is, TARGET is where it is going.
        # Driven here rather than by a widget binding because these cells are
        # plain QLabels (see _target_column).
        for a, lab in getattr(self, '_target_lbl', {}).items():
            try:
                i = self.AXIS_IDX[a]
                lab.setText('{:+.2f}'.format(
                    st.position[i] - st.g5x_offset[i] - st.g92_offset[i]
                    - st.tool_offset[i]))
            except Exception:
                pass
