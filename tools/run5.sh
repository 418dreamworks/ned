...
out += ['',
 ...
 '[AXIS_B]',
 'MAX_VELOCITY = 90',
 '# ACCELERATION MEASURED 2026-08-14, chucks only, no cutter.',
 '# B still turned at 30720 deg/s2 and STALLED at 61440, so the',
 '# unloaded ceiling sits between them. Operator took 75 percent of',
 '# the highest confirmed value, 0.75 x 30720 = 23040, then halved',
 '# it to 11520 the same day (that is a bit too fast).',
 '#',
 '# It is a TORQUE ceiling, so a cut spends it: no loaded move ever',
 '# sees 11520. MAX_VELOCITY stays 90 -- we do not need it spinning',
 '# that quickly. At 11520, B reaches 90 deg/s in 7.8 ms over',
 '# 0.352 deg, so a 2.000 deg block is 87 percent cruise.',
 '#',
 '# On the 2.000 deg blocks the groove is posted in, the planner',
 '# stops at every boundary and the rate is sqrt(2*a*d)/2 capped by',
 '# MAX_VELOCITY: about 67 deg/s here, where 30 deg/s2 gave 5.5.',
 '# docs/commissioning/b_accel_ceiling.md',
 'MAX_ACCELERATION = 11520',
 ...
 'MAX_ACCELERATION = 11520',
 'STEPGEN_MAXVEL = 108',
 '# 1.5x MAX_ACCELERATION, the ratio this joint has always carried',
 '# (45 against 30). The stepgen needs headroom above the planner',
 '# or it clips the ramp the planner just asked for.',
 'STEPGEN_MAXACCEL = 17280',
