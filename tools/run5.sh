...
if [ "$NED_MODE" = "xyzab" ]; then
  ...
  awk '
    /^JOINTS = /            { print "JOINTS = 7"; next }
    /^KINEMATICS = /        { print "KINEMATICS = trivkins coordinates=XYZXACB kinstype=B"; next }
    /^COORDINATES = /       { print "COORDINATES = X Y Z X A C B"; next }
    /^POSTGUI_HALFILE = /   { print "POSTGUI_HALFILE = postgui_ab_all.hal"; next }
    /^HALFILE = \.\.\/ned5\/ned5_iron\.hal$/ {
        print
        print "# -xyzab overlay: workpiece rotary B on 7I85S stepgen.00/01."
        print "HALFILE = ../ned5/ned5_b.hal"
        next }
    { print }
    END {
      print ""
      print "# ===================== GENERATED BY run5.sh -xyzab ====================="
      print "# Hand-edits here are destroyed on the next launch. B has no encoder and"
      print "# no switches; the worm is self-locking, so it holds unpowered."
      print "[AXIS_B]"
      print "MAX_VELOCITY = 30"
      print "MAX_ACCELERATION = 300"
      print "# No soft limits on B (operator: continuous workpiece rotary)."
      print "MIN_LIMIT = -3600000"
      print "MAX_LIMIT = 3600000"
      print ""
      print "[JOINT_6]"
      print "TYPE = ANGULAR"
      print "MAX_VELOCITY = 30"
      print "MAX_ACCELERATION = 300"
      print "STEPGEN_MAXVEL = 36"
      print "STEPGEN_MAXACCEL = 450"
      print "# 22.222 steps/deg = 400 p/rev x 20 worm / 360 (docs/components.md:50)."
      print "# The two drives are mechanically mirrored, hence the sign flip."
      print "SCALE = 22.222"
      print "SCALE_MIRROR = -22.222"
      print "FERROR = 5.0"
      print "MIN_FERROR = 1.0"
      print "MIN_LIMIT = -3600000"
      print "MAX_LIMIT = 3600000"
      print "# In-place home: search and latch both zero, HOME == HOME_OFFSET, so"
      print "# the cycle sets the position and the final move is zero length."
      print "HOME = 0.0"
      print "HOME_OFFSET = 0.0"
      print "HOME_SEARCH_VEL = 0"
      print "HOME_LATCH_VEL = 0"
      print "HOME_USE_INDEX = NO"
      print "# No HOME_SEQUENCE: B stays out of Home All, like A and C."
    }' "$INI" > "$GEN_AB" || { echo "run5: ab ini generation FAILED"; exit 1; }
  INI="$GEN_AB"
