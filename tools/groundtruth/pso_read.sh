...
# Requires Pn515=8882 (SEN mode) on both head drives.  Usage:  tools/groundtruth/pso_read.sh A|C
#
# Sequence (order is crucial -- else C leaks in):
#   RX listening  ->  R4 to target (OUTPUT5: 0=C via NC, 1=A via NO)  ->  SEN OFF->ON pulse.
# SEN is sent LAST, after R4 settles; a 1 s listen window catches the streamed frames.
set -e
DRIVE=${1:-C}
case "$DRIVE" in
  A|a) R4=1; LABEL=A ;;
...
timeout 40 halrun -f "$HAL" > "$LOG" 2>&1 || true
halrun -U >/dev/null 2>&1 || true
