...
2026-08-01) + pendant->screen sync + core-section hiding. The TOOLPROBE
button is gone from the .ui (PB's stock controls serve the tool probe);
its HAL plumbing stays for the postgui nets.
...
class UserTab(QWidget):
    def __init__(self, parent=None):
        ...
        # display (baseline: 9000 in MAN, program S in MDI/AUTO). FSR
        # sliders are STOCK (button-cluster experiment reverted).
        ...
        # GUI NUMBERING (operator 2026-08-01): every interactive control
        # gets a small numbered badge; the number->objectName map is dumped
        # to ned/gui_map.txt so "change #37" is unambiguous. Runs late so
        # the DEFERRED user DROs are included.
    ...
    # ---- JOG & PRESETS panel (v2, operator spec 2026-08-01 23:14) ---------
    ...
    # SOFT-LIMIT PRE-CHECK (reject, never clamp) -> switch to MDI -> CONFIRM
    # task_mode == MDI by POLLING (ned_brain auto-restores MANUAL on an edge;
    # losing that race paints "Must be in MDI mode" toasts) -> ONE c.mdi()
    # FIRE-AND-FORGET. NEVER wait_complete() around motion: it times out
    # silently after ~5 s and a mode switch ABORTS in-flight motion. The
    # brain hands back MANUAL + teleop ~1 s after motion truly completes.
    #
    # RELATIVE strategy (GO REL and the Z +10 preset): this panel NEVER
    # issues G91. Relative intent is converted to an ABSOLUTE target
    # (current work position + delta, house offset math) and sent as one
    # G90 G1 line -- an abort partway can never leave G91 modal. The v2
    # mockup card says "(G91)" for GO REL; the coordinator confirmed the
    # v1 delta-to-absolute conversion stands.
    #
    # v2 additions: split GO ABS / GO REL (no global ABS/REL mode); GOs
    # disabled until a field parses; Enter fires GO ABS; fields RETAIN
    # values after a move; status strip (dot + text + STOP=abort) at the
    # panel bottom; presets + GOs lock while motion runs.
    ...
    def _jog_wire(self):
        try:
            ...
            self._jog_status_timer.start(400)
    ...
    def _jog_enter(self):
        ...
        b = self.findChild(QWidget, 'jp_go_abs')
    ...
    def _jog_apply_enables(self):
        ...
        for name in ('jp_go_abs', 'jp_go_rel'):
            ...
            if b is not None and b.isEnabled() != want:
                b.setEnabled(want)
        b = self.findChild(QWidget, 'jp_stop')
        want = self._jog_idle is False
        if b is not None and b.isEnabled() != want:
            b.setEnabled(want)
    ...
    def _jog_status_tick(self):
        ...
        d = self.findChild(QWidget, 'jp_status_dot')
        t = self.findChild(QWidget, 'jp_status_text')
        if d is not None:
    ...
    # FSR override sliders: STOCK, untouched (the button-cluster idea was
    # tried 2026-08-01 and reverted -- "you completely fucked up FSR").
    ...
    def _tick(self):
        ...
        try:
            ...
            # V slider readout has NO units (StatusLabel max_vel_slider, its
            # text rule prints maxvel*60): put "mm/min" beside it.
            if not getattr(self, '_maxv_units_added', False):
                lbl = win.findChild(QWidget, 'max_vel_slider')
                lay = lbl.parentWidget().layout() if lbl is not None else None
                if lbl is not None and lay is not None:
                    from PySide6.QtWidgets import QLabel
                    u = QLabel('mm/min')
                    u.setStyleSheet('color: rgb(160,160,160); font: 8pt;')
                    idx = lay.indexOf(lbl)
                    try:
                        lay.insertWidget(idx + 1, u)
                    except Exception:
                        lay.addWidget(u)
                    self._maxv_units_added = True
                    LOG.info('V slider units label added (mm/min)')
