...
| [commissioning/tool_guard_boot_excuse_2026-08-07.md](commissioning/tool_guard_boot_excuse_2026-08-07.md) | The tool guard had a boot mute switch (brain.guard-arm) unmuted by a 25 s clock that started before power-on; deleted -- the guard now watches from HAL load, the boot mismatch locks jog+feed and clears itself. Alarm text is now CHECK SPINDLE RACK TABLE CONSISTENCY |
| [commissioning/head_home_pin_gate_2026-08-07.md](commissioning/head_home_pin_gate_2026-08-07.md) | Why C travelled −114.7° on a zero-travel declare (homing.c:1279 destination latch vs writes after cmd.home), the single-writer + read-back + HOME_IDLE gate that closes it, the motion detector, and the head-busy homing lockout |
