<!-- Mirror of https://linuxcnc.org/docs/html/hal/comp.html -->
...
Note  |  sudo (for root permission) is needed when using LinuxCNC from a deb package install. When using a Run-In-Place (RIP) build, root privileges should not be needed. |
...
To alter the allowed number of personality items for user-built components, use the *--personality* option with halcompile. For example, to allow up to 128 personality times:
